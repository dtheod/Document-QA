...
# from backend.indexing import index_documents # Not needed if we assume indexing is done or we skip it for now
...
async def run_ragas_evaluation():
    try:
        ...
        for query in TEST_QUESTIONS:
            ...
            # Extract contexts
            contexts = []
            if "retriever_results" in result_gen and isinstance(result_gen["retriever_results"], list):
                 contexts = [doc.page_content if hasattr(doc, 'page_content') else str(doc) for doc in result_gen["retriever_results"]]
