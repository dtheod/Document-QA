...
from app.services.rag.indexing import get_retriever
from huggingface_hub import InferenceClient
from app.utils import load_prompt_from_yaml, get_company_docs
from app.models.schemas import State, ClassificationOutput, JudgeOutput
import logging
...
class WorkflowAgents:
    ...
    def injection_agent(self, state: State) -> State:
        """
    ...
    def contextualize_agent(self, state: State) -> State:
        """
        ...
        # Load prompt
        contextualize_prompt = load_prompt_from_yaml(
            yml_type="rag_core.yml", prompt_id="contextualize_prompt_v1"
        )
    ...
    def classification_agent(self, state: State) -> State:
        """
        ...
        classification_prompt = load_prompt_from_yaml(
            yml_type="rag_core.yml", prompt_id="classification_prompt_v1"
        )
        ...
        logger.info(
            f"--- Classification Agent --- Company: {company}, Type: {query_type}"
        )
        return state

    def retriever_agent(self, state: State) -> State:
        query = state["query"]
        company = state["company"].lower()
        query_type = state["query_type"]
        logger.info(
            f"--- Retriever Agent --- Query: {query}, Company: {company}, Type: {query_type}"
        )
    ...
    def summarisation_agent(self, state: State) -> State:
        ...
        # Load prompt from YAML
        rag_prompt = load_prompt_from_yaml(
            yml_type="rag_core.yml", prompt_id="summarisation_prompt_v1"
        )
        ...
        if len(company_docs) > batch_size:
            ...
            batches = [
                company_docs[i : i + batch_size]
                for i in range(0, len(company_docs), batch_size)
            ]
            ...
            response = chain.invoke({"query": query, "context": company_docs})
    ...
    def qa_agent(self, state: State) -> State:
        """Stub for qa agent."""
        query = state["query"]
        ...
        # Load prompt from YAML
        rag_prompt = load_prompt_from_yaml(
            yml_type="rag_core.yml", prompt_id="qa_agent_v1"
        )

        # Prepare context from retrieved docs
        context_text = "\n\n".join([doc.page_content for doc in retriever_results])
        citations = "\n".join(
            [f"[{doc.metadata.get('id')}]" for doc in retriever_results]
        )

        chain = rag_prompt | self.llm
        response = chain.invoke(
            {"query": query, "context": context_text, "citations": citations}
        )
    ...
    def general_agent(self, state: State) -> State:
        ...
        # Load prompt from YAML
        rag_prompt = load_prompt_from_yaml(
            yml_type="rag_core.yml", prompt_id="general_agent_v1"
        )
    ...
    def judge_agent(self, state: State) -> State:
        ...
        # Load prompt from YAML
        rag_prompt = load_prompt_from_yaml(
            yml_type="rag_eval.yml", prompt_id="judge_agent_v1"
        )

        structured_llm = self.llm.with_structured_output(JudgeOutput)

        chain = rag_prompt | structured_llm
        result = chain.invoke(
            {"query": query, "answer": response, "retrieved_docs": retrieved_docs}
        )
        ...
        logger.info(
            f"--- Judge Agent --- Valid: {result.is_valid}, Feedback: {result.feedback}"
        )
        return state
